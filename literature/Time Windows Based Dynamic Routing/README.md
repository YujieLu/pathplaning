...
* 异常车辆需要占用路径！！！
* 异常车辆需要将对应任务重新分配？？暂时可以不做
* 进入任何一条路径之前，需要确定这条路径以及这条路径会碰撞的路径没有被预约


## 基于此方案之后与原系统差别
### 优点
* 所有路径已经预先处理过，可通行，并且在优先级高的任务确定的情况下时间最短的（通行顺序和路径选择是联合优化的，也可以扩展至车辆选择，优先级顺序要验证会比较麻烦）
* 不需要画管制区域，人工工作较少
...
## 小结
* 如果按照此方法来做，最大优势是无需画管制区域，plan方面优势是路径选择和通行顺序是联合优化的
* 任务优先级：单独优化
* 车辆选择：先单独优化
* 路径选择、通行顺序：联合优化
